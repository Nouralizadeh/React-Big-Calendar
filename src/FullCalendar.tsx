...
const SimpleFullCalendar: React.FC = () => {
  ...
  const handleUpdateEvent = (eventData: { title: string; start: Date; end: Date }) => {
    if (selectedEvent) {
  ...
  const handleEventClick = (clickInfo: any) => {
    // باز کردن modal/drawer برای ویرایش به جای confirm delete
    setSelectedEvent({
  ...
  const handleViewChange = (newView: 'timeGridWeek' | 'timeGridDay') => {
    if (calendarRef.current) {
      const calendarApi = calendarRef.current.getApi();
...
export default SimpleFullCalendar;